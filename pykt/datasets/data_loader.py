...
class KTDataset(Dataset):
    ...
    def __load_data__(self, sequence_path, folds, pad_val=-1):
        ...
        """
        dori = {"qseqs": [], "cseqs": [], "rseqs": [], "tseqs": [], "scl": [], "smasks": [], "stc":[], "smc":[], "score":[], "status":[], "problem_difficulty":[]}
        ...
        for i, row in df.iterrows():
            ...
            if "rs" in row:
                dori["rs"].append([int(_) for _ in row["rs"].split(",")])
            if "ts" in row:
                dori["ts"].append([int(_) for _ in row["ts"].split(",")])
            if "scl" in row:
                dori["scl"].append([int(_) for _ in row["scl"].split(",")])
            if "stc" in row:
                dori["stc"].append([int(_) for _ in row["stc"].split(",")])
            if "smc" in row:
                dori["smc"].append([int(_) for _ in row["smc"].split(",")])
            if "score" in row:
                dori["score"].append([int(_) for _ in row["score"].split(",")])
            if "status" in row:
                dori["status"].append([int(_) for _ in row["status"].split(",")])
            if "problem_difficulty" in row:
                dori["problem_difficulty"].append([int(_) for _ in row["problem_difficulty"].split(",")])
