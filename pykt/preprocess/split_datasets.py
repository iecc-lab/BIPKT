...
ALL_KEYS = ["fold", "uid", "questions", "concepts", "responses", "timestamps","slc","stc","smc","score",
            "status","problem_difficulty", "selectmasks", "is_repeat", "qidxs", "rest", "orirow", "cidxs"]
ONE_KEYS = ["fold", "uid"]
    ...
    with open(fname, "r", encoding="utf8") as fin:
        ...
        while i < len(lines):
            ...
            if i % 11 == 0:  # stuid
                ...
                dcur["uid"] = stuid
                goodnum += seq_len
            elif i % 11 == 1:  # question ids / names
                qs = []
                if line.find("NA") == -1:
                    effective_keys.add("questions")
                    qs = line.split(",")
                dcur["questions"] = qs
            elif i % 11 == 2:  # concept ids / names
                cs = []
                if line.find("NA") == -1:
                    effective_keys.add("concepts")
                    cs = line.split(",")
                dcur["concepts"] = cs
            elif i % 11 == 3:  # responses
                ...
                if line.find("NA") == -1:
                    ...
                    if not flag:
                        ...
                        continue
                dcur["responses"] = rs
            elif i % 11 == 4:  # timestamps
                ts = []
                if line.find("NA") == -1:
                    effective_keys.add("ts")
                    ts = line.split(",")
                dcur["ts"] = ts
            elif i % 11 == 5:  # usets
                scl = []
                if line.find("NA") == -1:
                    effective_keys.add("scl")
                    scl = line.split(",")
                dcur["scl"] = scl
                ...
                problem_difficulty = []
                if line.find("NA") == -1:
                    effective_keys.add("problem_difficulty")
                    problem_difficulty = line.split(",")
                dcur["problem_difficulty"] = problem_difficulty
...
def main(dname, fname, dataset_name, configf, min_seq_len=3, maxlen=200, kfold=5):
    ...
    Args:
        ...
        fname (str): the data file used to split, needs 11 columns, format is: (NA indicates the dataset has no corresponding info)
            uid,seqlen: 50121,4
            quetion ids: NA
