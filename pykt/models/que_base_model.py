...
class QueEmb(nn.Module):
    def __init__(self,num_q,num_c,emb_size,model_name,device='cpu',emb_type='iekt',emb_path="", pretrain_dim=768):
        """_summary_
        ...
        if emb_type == "bipkt":
            ...
            # self.que_emb.weight.requires_grad = False
            self.concept_emb = nn.Parameter(torch.randn(self.num_c, self.emb_size).to(device), requires_grad=True) #concept embeding
            self.que_c_linear = nn.Linear(self.emb_size,self.emb_size)
            self.pro_change = nn.Parameter(torch.randn(self.num_c, self.emb_size).to(device), requires_grad=True)
            self.pro_diff = nn.Embedding(self.num_q, self.emb_size)
    ...
    def get_avg_skill_emb(self,c):
        ...
        concept_avg = (concept_emb_sum.to(self.device) / concept_num.to(self.device))


        return concept_avg

    def get_avg_diff_skill_emb(self, c):

        # add zero for padding
        concept_emb_cat = torch.cat(
            [torch.zeros(1, self.emb_size).to(self.device),
             self.pro_change], dim=0)
        # shift c
    ...
    def forward(self,q,c,r=None):
        ...
        if emb_type == "qaid":
            ...
            emb_qca = torch.cat([emb_qc.mul((1-r).unsqueeze(-1).repeat(1,1, self.emb_size * 2)),
                                emb_qc.mul((r).unsqueeze(-1).repeat(1,1, self.emb_size * 2))], dim = -1)# s_t 扩展，分别对应正确的错误的情�?
